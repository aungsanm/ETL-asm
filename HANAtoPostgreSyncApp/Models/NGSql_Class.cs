...
{
    public class NGSql_Class : IDisposable
    {
        
        private string ConnestionString;

        public NGSql_Class(string _constr)
        {
        ...
        NpgsqlDataAdapter objAdpter = new NpgsqlDataAdapter();
        ...
        private void BeginTranscation()
        {
            ...
            //objConnection = new NpgsqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["NpgsqlConnection"].ToString());
            objConnection = new NpgsqlConnection(ConnestionString);
            ...
            try
            {
                ...
                if (ParameterCount > 0)
                {
                    ...
                    for (Pcount = 0; Pcount <= ParameterCount - 1; Pcount++)
                    {
                        ...
                        {
                            objCommand.Parameters.Add(ParaMeterNm[Pcount], NpgsqlTypes.NpgsqlDbType.Varchar, 5000).Direction = ParameterDirection.Output;
                        }
                        else
                        {
                            objCommand.Parameters.AddWithValue(ParaMeterNm[Pcount], ParaMeterVal[Pcount]);
            ...
            try
            {
                ...
                if (ParameterCount > 0)
                {
                    ...
                    for (Pcount = 0; Pcount <= ParameterCount - 1; Pcount++)
                    {
                        if (ParaMeterNm[Pcount] == "?")
                        {
                            objCommand.Parameters.Add(ParaMeterNm[Pcount], NpgsqlTypes.NpgsqlDbType.Varchar).Direction = ParameterDirection.Output;
                        }
                        else
                        {
                            objCommand.Parameters.AddWithValue(ParaMeterNm[Pcount], ParaMeterVal[Pcount]);
